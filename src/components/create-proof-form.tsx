...
/**
 * Core create-proof form — file drop → hash → register → receipt.
 * No hero header, no history section. Meant to be embedded in any layout.
 */
export function CreateProofForm() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status line */}
      {statusText && !receipt ? (
        <p aria-live="polite" className={`text-sm font-medium ${hashError ? "text-error" : isPending || isConfirming || isChecking ? "text-accent" : "text-text-muted"}`}>
          {statusText}
        </p>
      ) : null}
      ...
      {/* Preflight */}
      {preflightMsg ? (
        <p aria-live="polite" className="text-sm text-accent">{preflightMsg}</p>
      ) : null}

      {/* Tx error */}
      {error ? (
        <p aria-live="assertive" className="text-sm text-error">{normalizeClientError(error, genericWalletErrorMessage)}</p>
      ) : null}
      ...
      {/* ── Ticket / Receipt ───────────────────────── */}
      {receipt ? (
        <div className="border-t-2 border-border-default pt-10">
